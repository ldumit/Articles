...
public static class ReflectionExtensions
{
    ...
    static readonly Dictionary<Type, string> CSharpTypeAliases = new Dictionary<Type, string>
        {
            { typeof(int), "int" },
            { typeof(short), "short" },
            { typeof(long), "long" },
    ...
    {
        if (!@this.GetGenericArguments().HasAny()) return @this.Name;
    ...
    public static T TryGetValue<T>(this PropertyInfo @this, object @object, T defaultValue = default(T))
    {
        try
        {
            var value = @this.GetValue(@object, null);
            if (value == null)
                value = defaultValue;
            return (T)value;
    ...
    public static T Cache<T>(this MethodBase @this, object[] arguments, Method<T> methodBody) where T : class =>
				Cache(@this, null, arguments, methodBody);
    ...
    public static IEnumerable<Type> SelectTypesByAttribute<T>(this Assembly @this, bool inherit)
        where T : Attribute => @this.GetExportedTypes().Where(t => t.IsDefined(typeof(T), inherit));
    ...
    public static PropertyInfo SafeGetProperty(this Type @this, string propertyName)
    {
        ...
        if (propertyName.LacksValue()) throw new ArgumentNullException(nameof(propertyName));
